pipeline {
    ...
    stages {
        ...
        stage('Push Docker Image') {
            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'sagadevelopa') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull sagadevelopa/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d sagadevelopa/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
